...
##If param $2=delete
# rm -rf ./services/$1
#update maria.gen_admin.integrations table set status=0 ie deleted
...
if [ "$ACTION" = "new" ]; then
  ...
  sed -i "/\/\/integrations/a const ${SERVICE_NAME}Router = require('./services/$SERVICE_NAME/routes');" app.js
