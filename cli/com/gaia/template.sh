...
tar -czvf /var/www/admin/uploads/$template-$newver.tar.gz /var/www/$domain/templates/$template

#2) ssh send file to repo/templates 
#cp /var/www/admin/uploads/$template-$newver.tar.gz /var/www/api/repo/templates
sudo sshpass -p $pass rsync -zvh /var/www/admin/uploads/$template-$newver.tar.gz dros@$ip:/var/www/api/repo/templates --log-file=update.log
