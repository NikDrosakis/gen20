...
WHERE book.id=?",array($G['id']));
    ...
    $img=$sel['img']=='' ? $bookdefaultimg: $G['SITE_URL'].'media/'.$sel['img'];
    ?>
...
<img id="bookimg" src="<?=$img?>" style="width:100%">
</div>
...
<?php foreach($book_status as $statusid => $statusval){ ?>
...
<?php foreach($isread as $readid => $readval){ ?>
