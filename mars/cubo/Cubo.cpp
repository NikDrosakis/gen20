...
bool Cubo::setupCubo(const std::string& name) {
    try {
        ...
        // Step 2: Insert cubo metadata into `pagegrp`
        std::map<std::string, std::string> pagegrpData = {
        ...
        if (!maria.inse(publicdb + ".pagegrp", pagegrpData)) {
            throw std::runtime_error("Failed to insert into pagegrp");
        }
        ...
        if (setup["mains"]) {
            for (const auto& main : setup["mains"]) {
                std::map<std::string, std::string> mainData = {
                    {"pagegrpid", std::to_string(insertedGrpId)},
                ...
                // Step 4: Insert into `pagecubo` table
                std::map<std::string, std::string> pagecuboData = {
                    {"mainid", std::to_string(insertedMainId)},
                    {"area", "m"},
                    {"cuboid", std::to_string(insertedGrpId)},
                    {"name", cuboName}
                };
                if (!maria.inse(publicdb + ".pagecubo", pagecuboData)) {
                    throw std::runtime_error("Failed to insert into pagecubo");
                }
            }
        }
