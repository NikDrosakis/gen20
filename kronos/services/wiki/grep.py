...
mylist = []
def line(q):
    for name in listdir("/var/www/store/texts"):
        with open('/var/www/store/texts/'+name, 'r') as f:
...
#            for i, line in enumerate(f):
                ...
                for ql in qcleanlist:                                    
                    if ql in linecleanlist:
                        mylist.append('<b>'+name+'</b>'+lineclean.replace(ql, "<span style='background:yellow'>"+ql+'</span>'))
    return mylist