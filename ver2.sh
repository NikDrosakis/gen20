...
NEW_VERSION=$(sudo mysql -u"$DB_USER" -p"$DB_PASS" -D"$DB_ADMINAME" -h"$DB_HOST" -se "SELECT MAX(version_tag) + 0.001 FROM versions")

# Check if NEW_VERSION is empty or not a valid decimal
if ! [[ "$NEW_VERSION" =~ ^[0-9]+(\.[0-9]+){1,2}$ ]]; then
    echo "[ERROR] Failed to get the new version number. Exiting."
    exit 1
fi
...
if  curl -sSL https://github.com/nikosdrosakis.gpg | gpg --import; then
  echo "[INFO] GPG Key imported successfully"
else
  echo "[ERROR] GPG Key import failed. Please verify GPG key setup"
    exit 1
fi
...
# TODO: Add and push commit with tag for the new version
...
if sudo mysql -u"$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -D"$DB_ADMINAME" -e "$MYSQL_QUERY"; then
    echo "Git pushed, versioning completed successfully."
else
  echo "[ERROR] Inserting the new version into database failed"
  exit 1
fi
...
sudo mariadb-dump -u "$DB_USER" -p"$DB_PASS" --no-data --triggers --routines --events --create-options --no-set-names --add-drop-table=false "$DB_ADMINAME" > "$SQL_FILE"
sudo mariadb-dump -u "$DB_USER" -p"$DB_PASS" --no-set-names --triggers --routines --events --create-options --add-drop-table=false "$DB_ADMINAME" > "$ADMIN_SQL_FILE"
...
if [ ! -e "$ADMIN_SQL_FILE" ] || [ ! -w "$ADMIN_SQL_FILE" ]; then
  echo "File does not exist or is not writable: $ADMIN_SQL_FILE"
  exit 1
else
  echo "SQL admin file $ADMIN_SQL_FILE created"
fi

# TODO: Backup the filesystem excluding certain directories
# Backup the filesystem, excluding directories like node_modules
sudo tar --exclude='*/node_modules/*' --exclude='./genenv' -czf "/var/gs/backup/gen_backup_${NEW_VERSION}.tar.gz" /var/www/gs
